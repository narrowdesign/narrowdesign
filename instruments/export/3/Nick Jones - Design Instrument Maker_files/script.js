...
function init() {
  ...
  */
  ...
  function animateRecording(i, then) {
    ...
    if (!checkIfPaused()) {
      ...
      if (step[0] === 'new') {
        ...
        if (propName.includes('-top') && recording[stepIndex + 1][2].includes('-bottom') && recording[stepIndex + 2][2].includes('-right') && recording[stepIndex + 3][2].includes('-left')) {
          for (let s = 0; s < 3; s++) {
            stepIndex++;
  ...
  function updateStyleDisplay() {
    ...
    const propName = userState.activePropName;
    registerViewChange();
    displayEl.style.transform = `translateY(${
      Math.max(0, Math.min(elTop, windowState.height - 30))
    }px)`;
    if (userState.isEditMode) return;
  ...
  function enterEditMode() {
    ...
    displayValueEl.innerText = "(Esc to exit)";
    elements.forEach((el) => {
      if (el !== bodyEl && !el.classList.contains('container') && !el.classList.contains('image')) {
