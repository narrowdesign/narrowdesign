...
function init() {
  ...
  */
  ...
  function displayScrollIndicator(e) {
    scrollIndicatorEl.style.transform = `translateX(-50%) rotate(${userState.wheelRotation * 0.1}deg)`;
    bodyEl.classList.add('isScrolling');
    clearTimeout(userState.scrollTimeout);
    userState.scrollTimeout = setTimeout(() => {
      bodyEl.classList.remove('isScrolling');
    }, 200);
  }

  function displayKeyPressed(keys) {
    if (keyDisplayEl.innerText === keys) return;
    keyDisplayKeyEls[0].innerText = keys.toUpperCase();
