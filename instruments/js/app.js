...
const unitRegex =
  /vw$|vh$|vmax$|dvh$|dvw$|svh$|svw$|lvh$|lvw$|dvmin$|dvmax$|lvmin$|lvmax$|svmin$|svmax$|vmin$|px$|rem$|em$|%$/;
...
const displayElementsUnderCursorListEl = document.querySelector(
  ".AppStyleDisplay__elementsUnderCursorList"
);
const displayInspectStyleListEl = document.querySelector(
  ".AppStyleDisplay__inspectStyleList"
);
...
let elements = [bodyEl, ...bodyEl.querySelectorAll("*")];
// Remove app elements
...
function init() {
  ...
  */
  ...
  const keyToProp = {
    as: {
      name: "align-self",
      enumList: ["center", "flex-end", "stretch", "flex-start"],
    },
    ai: {
      name: "align-items",
      enumList: [
    ...
    d: {
      name: "display",
      enumList: ["flex", "grid", "block", "inline-block", "inline"],
    },
    fb: {
      name: "--filter",
      min: 0,
    ...
    js: {
      name: "justify-self",
      enumList: ["flex-start", "center", "flex-end", "stretch"],
    },
    l: {
      name: "left",
      initialValue: "0vmax",
    ...
    ov: {
      name: "overflow",
      enumList: ["visible", "hidden", "auto", "scroll"],
    },
    p: {
      name: "padding",
      min: 0,
  ...
  function animateRecording(i, then) {
    ...
    if (!checkIfPaused()) {
      ...
      if (step[0] === "new") {
        ...
        const nameMatch = Object.keys(keyToProp).find(
          (key) =>
            keyToProp[key].name === propName ||
            keyToProp[key].name[0] === propName
        );
        if (nameMatch) {
  ...
  function checkIfPaused() {
    return (
      userState.isTypingDelay ||
      userState.isListDelay ||
      userState.isReadingDelay
    );
  }
  ...
  function animateTyping(el, text) {
    ...
    if (
      elTop + 100 > window.scrollY + windowState.height ||
  ...
  function displayScrollDisplay(e) {
    scrollDisplayEl.style.transform = `translateX(-50%) rotate(${
      userState.wheelRotation * 0.1
    }deg)`;
  ...
  function enterClassListMode() {
    ...
    userState.classListTimeout = setTimeout(() => {
      classListEl.focus();
      const classList = userState.selectedElement.classList.value
        .split(" ")
        .filter(
          (className) =>
            !className.includes("AppElement") && !className.includes("App--")
        )
        .join(" ");
  ...
  function handleWindowClick(e) {
    ...
    if (userState.isShiftKey) {
      ...
      const elementsUnderCursor = document
  ...
  function handleKeyPress(e) {
    ...
    if (userState.isActivePropParamIndexAlt) {
      // ["!","@","#","$"] for color inside shadow
      return;
    } else if (parseInt(e.key) >= 1 && parseInt(e.key) <= 9) {
      // for props made of a list of values
    ...
    if (userState.firstKey) {
      ...
      const firstKeyMatch =
        Object.keys(keyToProp).filter((name) => name[0].includes(e.key))
          .length > 0;
      if (firstKeyMatch) {
  ...
  function handleSearchEnterKeyPress() {
    const valueMatch =
      userState.activeProp.enumList?.filter((val) => searchEl.value === val)
        .length > 0;
  ...
  function handleClassListEnterKeyPress() {
    const appClassList = userState.selectedElement.classList.value
      .split(" ")
      .filter(
        (className) =>
          className.includes("AppElement") || className.includes("App--")
    ...
    userState.selectedElement.classList = appClassList
      ? `${appClassList} ${val}`
      : val;
    exitClassListMode();
  }
  ...
  function handleKeyUp(e) {
    ...
    if (
      userState.isRecording &&
      userState.selectedElement &&
      userState.isEditMode
    ) {
      addToRecording([
        "type",
        elements.indexOf(userState.selectedElement),
        userState.selectedElement.innerHTML,
      ]);
      updateUndoStack({
        type: "type",
  ...
  function handlePaste(e) {
    // cancel paste
    if (
      userState.isSearchMode ||
      userState.isClassListMode ||
      userState.isLinkInputMode
    )
      return;
    e.preventDefault();
  ...
  function setSelectedElement(el) {
    ...
    document
      .querySelector(".AppElement--isSelected")
      ?.classList.remove("AppElement--isSelected");
    ...
    if (userState.selectedElement?.classList.contains("container")) {
      ...
      userState.activeContainer =
        userState.selectedElement.closest(".container");
    }
    redrawGrid();
    scrollIntoView(el);
  }
  ...
  function setMultiSelectedElement(el) {
    ...
    if (userState.multiSelectedElementList.includes(el)) {
      userState.multiSelectedElementList.splice(
        userState.multiSelectedElementList.indexOf(el),
        1
      );
      el.classList.remove("AppElement--isMultiSelected");
    } else {
  ...
  function selectNextElement(change) {
    if (userState.isEditMode) {
      ...
      let newIndex =
        editableElements.indexOf(userState.selectedElement) + change;
  ...
  function resetActivePropValue() {
    if (userState.activeProp.axis) {
      ...
      setStyleProperty(userState.selectedElement, userState.activePropName, "");
    }
    updateStyleDisplay();
    redrawGrid();
  }
  ...
  async function setActiveProp(key, propIndex) {
    ...
    if (userState.activePropName === "border-radius") {
      [
        ...
        userState.activeProp.max = Math.max(
          userState.activeProp.max,
  ...
  function updateStyleProp(delta) {
    ...
    const computedValue = computedStyle.getPropertyValue(`${propName}`);
    let val = propValue
      ? propValue.trim().split(" ")
      : computedValue.trim().split(" ");
  ...
  function updateEnumList(enumList) {
    ...
    enumList.forEach((value, i) => {
      if (
        getComputedStyle(userState.selectedElement)
    ...
    displayEnumListEl
      .querySelectorAll(".AppStyleDisplay__enum")
      .forEach((el, i) => {
  ...
  function setStyleProperty(el, propName, newVal) {
    ...
    undoable.push({
      ...
      el: userState.selectedElement,
      propName,
      value: getComputedStyle(userState.selectedElement).getPropertyValue(
        propName
      ),
    });
    el.style.setProperty(propName, newVal);
    ...
    if (userState.multiSelectedElementList.length > 0) {
      userState.multiSelectedElementList.forEach((multiEl) => {
        ...
        multiEl.style.setProperty(propName, newVal);
      });
    ...
    if (propName.substring(0, 2) === "--") {
      ...
      el.style.setProperty(propName.substring(2), newVal);

      userState.multiSelectedElementList.forEach((multiEl) => {
        multiEl.style.setProperty(propName.substring(2), newVal);
      });
    }
    // }
  }
  ...
  function updateStyleDisplay() {
    ...
    if (userState.selectedElement) {
      text =
        getComputedStyle(el)
          .getPropertyValue(`${userState.activePropName}`)
          .trim() || userState.activeProp.initialValue;
    }
    ...
    valuesArray.forEach((value, i) => {
      const className =
        i === userState.activePropParamIndex
          ? "AppStyleDisplay__value--isActive"
          : "";
  ...
  function setMultipleSideProps(
    el = userState.selectedElement,
    prop = userState.activePropName,
    newVal,
    axis = null
  ) {
    console.log(newVal);
    ...
    // );
    if (axis !== "x") {
      setStyleProperty(el, `${prop}-top${width}`, `${newVal}`);
      setStyleProperty(el, `${prop}-bottom${width}`, `${newVal}`);
    }
    if (axis !== "y") {
      setStyleProperty(el, `${prop}-right${width}`, `${newVal}`);
      setStyleProperty(el, `${prop}-left${width}`, `${newVal}`);
    }
  }
  ...
  function duplicateElement(element, container, key) {
    ...
    userState.firstKey = undefined;
    let rowStart = parseInt(
      getComputedStyle(element).getPropertyValue("grid-row-start")
    );
    let rowEnd = parseInt(
      getComputedStyle(element).getPropertyValue("grid-row-end")
    );
    let columnStart = parseInt(
      getComputedStyle(element).getPropertyValue("grid-column-start")
    );
    let columnEnd = parseInt(
      getComputedStyle(element).getPropertyValue("grid-column-end")
    );
    ...
    newElement.querySelectorAll("*").forEach((el) => {
      if (
        !el.classList.contains("AppGridDisplay__line") &&
        !el.classList.contains("AppGridDisplay")
      ) {
        console.log(el);
        elements.push(el);
      }
  ...
  function createNewElement(
    key,
    container = userState.activeContainer || bodyEl,
    text,
    src
  ) {
    userState.firstKey = undefined;
  ...
  function createNewImage(src) {
    const searchTerm = userState.isAnimating
      ? src
      : prompt("Insert an image of a/an");
  ...
  function createNewVideo(src) {
    const searchTerm = userState.isAnimating
      ? src
      : prompt("Insert a link to a video");
  ...
  function deleteElement(el, deleteChildren) {
    ...
    const children = Array.from(el.children).filter(
      (el) => !el.classList.contains("AppGridDisplay")
    );
    if (
      children.length === 0 ||
      deleteChildren ||
      confirm("Delete children?")
    ) {
      el.remove();
  ...
  function getHSLA(delta) {
    let colorVal = `${getComputedStyle(
      userState.selectedElement
    ).getPropertyValue(`${userState.activePropName}`)}`.trim();
    if (colorVal === "transparent") {
      colorVal = `${getComputedStyle(
        userState.selectedElement.parentNode
      ).getPropertyValue(`${userState.activePropName}`)}`.trim();
      if (colorVal === "transparent") {
  ...
  function rgbToHSLA(colorVal) {
    ...
    return `hsla(${hVal.toFixed(5) / 1}deg, ${sVal.toFixed(5) / 1}%, ${
      lVal.toFixed(5) / 1
    }%, ${a})`;
  }
  ...
  function getShadow(delta) {
    ...
    let newVal =
      parseFloat(props[userState.activePropParamIndex + 1], 10).toFixed(5) / 1 +
      delta * 0.1;
    ...
    if (userState.isActivePropParamIndexAlt) {
      ...
      props[userState.activePropParamIndex + 1] = `${newVal}${unit}`;
    }
    return props.join(" ");
  }

  function getTransform(delta) {
    let transformVal = `${getComputedStyle(userState.selectedElement)
      .getPropertyValue(`${userState.activePropName}`)
      .trim()}`;
    transformVal = transformVal.replace(/\s{2,}/g, " ");
    const transformProps = transformVal.split(" ");
    const index = userState.isActivePropParamIndexAlt
      ? userState.activePropParamIndex + 9
      : userState.activePropParamIndex;
    const activeProp = transformProps[index];
    const name = activeProp.substring(0, activeProp.indexOf("("));
    const propVal = activeProp.substring(
      activeProp.indexOf("(") + 1,
      activeProp.indexOf(")")
    );
  ...
  function redrawGrid(container = userState.activeContainer, recursive) {
    ...
    columns.forEach((column) => {
      let columnWidth =
        column !== "none" ? `${parseFloat(column, 10)}` : parseFloat(width, 10);
    ...
    rows.forEach((row) => {
      let rowHeight =
        row !== "none" ? `${parseFloat(row, 10)}` : parseFloat(height, 10);
  ...
  function undo() {
    if (userState.undoStack.length > 0) {
      ...
      while (i >= 0) {
        if (
          recording[i][0] === "select" ||
          recording[i][0] === "multi-select" ||
          (recording[i][0] === "style" &&
            recording[i][2] === prev.propName &&
            elements[recording[i][1]] === prev.el)
        ) {
          recording.pop();
        } else if (
          (recording[i][0] === "new" || recording[i][0] === "duplicate") &&
          prev.type === "new"
        ) {
          recording.pop();
          recording.pop();
